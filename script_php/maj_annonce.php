            ...
            if ($titreAnnonce&&$description&&$typeAnnonce) {
                if ($typeAnnonce == 'Alternance' || $typeAnnonce == 'stage') {
                    if (is_numeric($remuneration)) {
                        $req = $bdd->exec("UPDATE annoncesentreprises SET titre='$titreAnnonce', description='$description', typeAnnonce='$typeAnnonce', remuneration='$remuneration' WHERE entreprise='{$dataEntreprise['id']}'");
                        ?><meta http-equiv="refresh" content="2;URL=/editannonce/<?php echo $dataAllAnnonce['id']; ?>" /></head><?php
